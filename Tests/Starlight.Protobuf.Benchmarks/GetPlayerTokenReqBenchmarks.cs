using BenchmarkDotNet.Attributes;
using BenchmarkDotNet.Configs;
using Google.Protobuf;
using Starlight.Game.Protocol;
using Starlight.Game.Protocol.V66;
using Starlight.Protobuf.Core;
using GGoogle = Starlight.Protobuf.Benchmarks.Google;

namespace Starlight.Protobuf.Benchmarks;

/// <summary>Realistic string/varint-heavy login packet (large field numbers).
/// Each op (de)serializes a batch of <see cref="N"/> messages.</summary>
[MemoryDiagnoser]
[GroupBenchmarksBy(BenchmarkLogicalGroupRule.ByCategory)]
[CategoriesColumn]
public class GetPlayerTokenReqBenchmarks
{
    private static readonly GetPlayerTokenReqSerializer Serializer = GetPlayerTokenReqSerializer.Instance;

    [Params(1, 1000)]
    public int N;

    private GGoogle.GetPlayerTokenReq[] _google = [];
    private GetPlayerTokenReq[] _starlight = [];
    private byte[][] _googleBytes = [];
    private byte[][] _starlightBytes = [];

    [GlobalSetup]
    public void Setup()
    {
        _google = Samples.Batch(N, Samples.GoogleToken);
        _starlight = Samples.Batch(N, Samples.StarlightToken);
        _googleBytes = Array.ConvertAll(_google, m => m.ToByteArray());
        _starlightBytes = Array.ConvertAll(_starlight, m => m.ToByteArray(Serializer));
    }

    [BenchmarkCategory("Serialize"), Benchmark(Baseline = true)]
    public long Google_Serialize()
    {
        long total = 0;
        foreach (var message in _google) total += message.ToByteArray().Length;
        return total;
    }

    [BenchmarkCategory("Serialize"), Benchmark]
    public long Starlight_Serialize()
    {
        long total = 0;
        foreach (var message in _starlight) total += message.ToByteArray(Serializer).Length;
        return total;
    }

    [BenchmarkCategory("Deserialize"), Benchmark(Baseline = true)]
    public long Google_Deserialize()
    {
        long total = 0;
        foreach (var bytes in _googleBytes) total += GGoogle.GetPlayerTokenReq.Parser.ParseFrom(bytes).Uid;
        return total;
    }

    [BenchmarkCategory("Deserialize"), Benchmark]
    public long Starlight_Deserialize()
    {
        long total = 0;
        foreach (var bytes in _starlightBytes)
        {
            var message = new GetPlayerTokenReq();
            message.MergeFrom(Serializer, bytes);
            total += message.Uid;
        }
        return total;
    }
}
